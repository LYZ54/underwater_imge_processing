...
def sharpen(img):
    ...
    gauss = cv.GaussianBlur(img, (5, 5), 3)
    n = 1
    unSharpMask = img - gauss
    stretchIm = hist_normalization(unSharpMask)
    result = (img + n * stretchIm) / 2
...
def laplace(image_path):
    ...
    image_l = np.double(image[:, :, 0]) / 256  # 取l
    # image_l = np.double(image[:, :, 0])
...
def saturation(image_path):
    ...
    out_img = np.ones((image.shape[0], image.shape[1]), np.float32)  # 初始化一个和图像大小一样的灰度图，每个像素为1
...
# print(1)
# print(type(img_compen))
# print(img_compen.dtype)
...
# print(2)
# print(type(img_gamma_adjust))
# print(img_gamma_adjust.dtype)
cv.imshow('img_gamma_adjust', img_gamma_adjust)
cv.waitKey(0)
# compare_show(img_compen, img_gamma_adjust)                # 图片对比

img_sharpen1 = sharpen(img_compen)  # 锐化1
# print(3)
# print(type(img_sharpen1))
# print(img_sharpen1.dtype)
# print(img_sharpen1.shape)
cv.imshow('img_sharpen', img_sharpen1)
cv.waitKey(0)
...
# print(img_lap_1)
# print(cv.countNonZero(img_lap_1))
# print(4)
# print(type(img_lap_1))
# print(img_lap_1.dtype)
# print("calculating sharpen")
img_lap_2 = laplace(img_sharpen1)  # 计算拉普拉斯权重2
# print(5)
# print(type(img_lap_2))
# print(img_lap_2.dtype)
cv.imshow('Laplace Image', img_lap_1)
cv.waitKey(0)
...
# print(img_wsal_1)
# print(6)
# print(type(img_wsal_1))
# print(img_wsal_1.dtype)
img_wsal_2 = saliency(img_sharpen1)  # 计算显著性权重2
# print(7)
# print(type(img_wsal_2))
# print(img_wsal_2.dtype)
# img_wsal_1 = img_wsal_1.astype(np.float32)
# img_wsal_2 = img_wsal_2.astype(np.float32)
cv.imshow('wsal Image1', img_wsal_1.astype(np.uint8))
cv.waitKey(0)
cv.imshow('wsal Image2', img_wsal_2.astype(np.uint8))
cv.waitKey(0)

img_wsat_1 = saturation(img_gamma_adjust)  # 计算饱和权重1
# print(img_wsat_1)
# print(8)
# print(type(img_wsat_1))
# print(img_wsat_1.dtype)
img_wsat_2 = saturation(img_sharpen1)  # 计算饱和权重2
# print(9)
# print(type(img_wsat_2))
# print(img_wsat_2.dtype)
cv.imshow('wsat Image1', img_wsat_1)
cv.waitKey(0)
cv.imshow('wsat Image2', img_wsat_2)
cv.waitKey(0)
...
# WSat 饱和权重
# 权重归一化处理
weight1 = (img_lap_1 + img_wsal_1 + img_wsat_1 + 0.1) / (
        img_lap_1 + img_wsal_1 + img_wsat_1 + img_lap_2 + img_wsal_2 + img_wsat_2 + 0.2)
weight2 = (img_lap_2 + img_wsal_2 + img_wsat_2 + 0.1) / (
        img_lap_1 + img_wsal_1 + img_wsat_1 + img_lap_2 + img_wsal_2 + img_wsat_2 + 0.2)
# print(10)
# print(type(weight1))
# print(weight1.dtype)
